---
title: "How fyrer Executes Tasks Across Your Monorepo"
...
<Steps>
  ...
  <Step title="Output streaming">
    As each task runs, its stdout and stderr are streamed in real time. In plain mode (`-n`), every line is prefixed with the `package:task` label and colorized so interleaved output from concurrent tasks remains easy to read. The default TUI mode routes logs into a per-task pane you can browse interactively.
  </Step>
